...
class Metronome():
    ...
    def __init__(self, bpm, **kwargs):
        self.BPM = bpm
        if kwargs['subdivide'] == None:
            self.subdivide = 1
        else:
            self.subdivide = kwargs['subdivide']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run a metronome')
    parser.add_argument('bpm', type=int)
    parser.add_argument('--subdivide', type=int)

    args = parser.parse_args()
    m = Metronome(**vars(args))
    m.start()
    while m.is_playing():
        time.sleep(.1)
    m.end()
